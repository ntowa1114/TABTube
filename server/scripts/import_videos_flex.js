...
function parseTitle(title) {
  ...
  // パターンB: アーティスト「曲名」形式（スラッシュが鍵括弧より前にない場合に適用）
  ...
  if (quoteMatch && (slashIdx === -1 || quoteIdx < slashIdx)) {
    ...
    // パターンA: 曲名 / アーティスト名 形式
    songTitle = cleanSong(afterPrefix.slice(0, slashIdx));
    artistName = cleanArtist(afterPrefix.slice(slashIdx + 1));
  } else {
    return null;
  }
...
async function main() {
  ...
  if (!handle) {
    console.error('使い方: node scripts/import_videos_flex.js "@チャンネルハンドル"');
    process.exit(1);
  }
  if (!YOUTUBE_API_KEY) {
