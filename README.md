...
ユーザーはYouTubeのTAB譜動画を検索・閲覧して、ギターやベースの練習を効率的に行うことができます。また、自分で新しいTAB譜動画を登録してライブラリを拡張することも可能です。楽器（ギター/ベース）や曲名、アーティスト名でフィルタリングして、目的の練習動画を素早く見つけられるサービスです。Googleアカウントでログインすることで、お気に入り登録やプレイリスト管理といったパーソナライズ機能も利用できます。
...
| ログイン | マイページ |
| :---: | :---: |
| <img src="[GIF/画像URL]" width="320"> | <img src="[GIF/画像URL]" width="320"> |
| <p align="left">Googleアカウントでワンクリックログイン</p> | <p align="left">お気に入り一覧・プレイリスト一覧を管理</p> |

Supabase Authを使用したGoogleOAuth認証を実装しています。ログイン状態に応じてUIが切り替わり、未ログインユーザーがお気に入り・プレイリスト機能を使おうとするとログインモーダルが表示されます。
...
| :---: | :---: |
| <img src="[GIF/画像URL]" width="320"> | <img src="[GIF/画像URL]" width="320"> |
| <p align="left">動画カードの☆ボタンでお気に入り登録・解除</p> | <p align="left">マイページでお気に入り動画を一覧表示</p> |

楽観的更新により、APIレスポンスを待たずにUIが即座に反映されます。SupabaseのRLS（行レベルセキュリティ）により、自分のデータのみ操作可能です。
...
| :---: | :---: |
| <img src="[GIF/画像URL]" width="320"> | <img src="[GIF/画像URL]" width="320"> |
| <p align="left">動画カードの「＋」ボタンで既存プレイリストへの追加・新規作成が可能</p> | <p align="left">プレイリスト内の動画を並び替え・削除、お気に入りから追加</p> |

ホーム画面・マイページのどちらからでもプレイリストに動画を追加できます。プレイリスト詳細ページでは動画の順序を↑↓ボタンで変更でき、お気に入り一覧から直接追加することも可能です。
...
| データベース | PostgreSQL (Neon) / Supabase (PostgreSQL) |
| 認証 | Supabase Auth（Google OAuth）/ @supabase/ssr 0.10.3 |
| CI/CD | （未実装） |
| インフラ | Vercel |
| その他 | Tailwind CSS v4 / react-player 3.4.0 / pg 8.x |
...
**【認証】**  
Supabase Authを使用したGoogle OAuthを実装しています。`@supabase/ssr`パッケージによりNext.jsのサーバーコンポーネント・Route Handler・Middlewareのすべてでセッションを正しく扱えるようにしています。MiddlewareでリクエストごとにJWTトークンの自動リフレッシュを行い、RLS（行レベルセキュリティ）と組み合わせることで、自分のデータのみ操作できる安全な設計を実現しています。
...
テーブル構成は大きく分けて、**「動画情報」「ユーザーデータ」** の2つに分類されます。

### 動画情報 に関するテーブル
`videos`テーブルでYouTube動画のメタデータを管理。`youtube_id`、`title`、`artist_name`、`instrument`、`created_at`カラムを持ち、楽器（ギター/ベース）ごとに同じ動画IDで別レコードとして登録可能な構造になっています。Neon（PostgreSQL）上に配置しています。

### ユーザーデータ に関するテーブル（Supabase）

`favorites`テーブルでユーザーごとのお気に入り動画を管理。`user_id`（Supabase Auth UID）と`youtube_id`の組み合わせにUNIQUE制約を設けることで重複登録を防いでいます。

`playlists`テーブルでユーザーが作成したプレイリストを管理。`playlist_videos`テーブルで各プレイリストに属する動画を管理し、`order_index`で再生順序を保持します。`playlist_videos`は動画のメタデータ（`title`・`artist_name`・`instrument`）も持つことで、`videos`テーブルへの依存なく動画情報を表示できる設計になっています。

すべてのユーザーデータテーブルにRLS（行レベルセキュリティ）を設定し、認証済みユーザーが自分のデータのみ操作できるよう保護しています。
...
- 動画の評価・コメント機能
- CI/CDパイプラインの構築
- ER図の更新（favorites・playlists・playlist_videosテーブルの追加）
- インフラ構成図の更新（Supabase・Neonの追加）
